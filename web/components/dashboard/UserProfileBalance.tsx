...
import { getCitizenVaultData, updateCitizenVaultBalance, type MockData } from '@/lib/mockDataService';

/**
 * User Profile and Balance Display Component
 * Displays citizen vault data from mockData.json
 * Updates $VIDA balance when PFF Scan succeeds
 */
export function UserProfileBalance() {
  const [vaultData, setVaultData] = useState(getCitizenVaultData());

  // Listen for balance updates from VitalizationScreen
  useEffect(() => {
    const handleBalanceUpdate = (event: CustomEvent) => {
      const newBalance = event.detail.balance;
      const updated = updateCitizenVaultBalance(newBalance);
      setVaultData(updated);
    };

    window.addEventListener('vidaBalanceUpdate' as any, handleBalanceUpdate as EventListener);
    return () => {
      window.removeEventListener('vidaBalanceUpdate' as any, handleBalanceUpdate as EventListener);
    };
  }, []);
      ...
      <div className="bg-[#16161a] rounded-xl p-6 border border-[#2a2a2e]">
        ...
        <div className="mb-6 p-4 bg-gradient-to-br from-[#c9a227]/20 to-[#e8c547]/10 rounded-lg border border-[#c9a227]/30">
          ...
          <p className="text-4xl font-bold text-[#e8c547] mb-1">
            {vaultData.spendable_balance_vida.toLocaleString('en-US', {
        ...
        <div className="space-y-4">
          <div className="p-4 bg-[#0d0d0f] rounded-lg border border-[#2a2a2e]">
            ...
            <p className="text-2xl font-bold text-[#c9a227]">
              {vaultData.total_vida_cap_minted.toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </p>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-base font-bold text-[#e8c547]">
                  {vaultData.split_records.personal_share_50.toLocaleString('en-US', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}{' '}
                  VIDA CAP
              ...
              <div className="flex items-center justify-between pt-2 border-t border-[#2a2a2e]">
                ...
                <span className="text-base font-bold text-[#6b6b70]">
                  {vaultData.split_records.state_contribution_50.toLocaleString('en-US', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}{' '}
                  VIDA CAP
        ...
        <div className="bg-[#16161a] rounded-xl p-6 border border-[#2a2a2e]">
          ...
          <div className="space-y-2">
            {vaultData.linked_bank_accounts.map((account, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 bg-[#0d0d0f] rounded-lg border border-[#2a2a2e]"
              >
