/**
 * PFF — Sequential Handshake Engine (local copy for mobile bundle).
 */
...
export function checkCohesionTimeout(startTime: number): boolean {
  return Date.now() - startTime > COHESION_TIMEOUT_MS;
}
  ...
  if (!data.bloodFlowDetected || data.livenessScore < 0.99) {
    return {
      ...
      error: {
        ...
        message: `Liveness score too low: ${data.livenessScore.toFixed(3)}`,
        phase: 'PHASE_1_VISUAL_LIVENESS',
        timestamp: Date.now(),
        hardwareError: false,
      },
  ...
  if (!data.fingerprintMatched || data.matchConfidence < 0.95) {
    return {
      ...
      error: {
        ...
        message: `Match confidence too low: ${data.matchConfidence.toFixed(3)}`,
        phase: 'PHASE_2_TACTILE_IDENTITY',
        timestamp: Date.now(),
        hardwareError: false,
      },
  ...
  if (!data.pulseDetected || data.heartbeatFrequency < 40 || data.heartbeatFrequency > 200) {
    return {
      ...
      error: {
        ...
        message: `Invalid heartbeat: ${data.heartbeatFrequency} BPM`,
        phase: 'PHASE_3_VITAL_PULSE',
        timestamp: Date.now(),
        hardwareError: true,
      },
...
export function verifyCohesion(state: SequentialHandshakeState): SequentialHandshakeResult {
  ...
  if (totalDuration > COHESION_TIMEOUT_MS) {
    return {
      ...
      error: {
        ...
        message: `Handshake exceeded 1.5s: ${totalDuration}ms`,
  ...
  if (!allPhasesComplete) {
    const failedPhase =
      ...
      state.phases.phase3?.status === 'failed' ? state.phases.phase3 : null;
    return {
      success: false,
      sessionId: state.sessionId,
      totalDuration,
...
export function flushBuffer(_state: SequentialHandshakeState): SequentialHandshakeState {
...
export async function executeSequentialHandshake(
  ...
  try {
    ...
    if (state.phases.phase1.status === 'failed') return verifyCohesion(state);
    if (checkCohesionTimeout(state.startTime)) return verifyCohesion(state);
    ...
    if (state.phases.phase2.status === 'failed') return verifyCohesion(state);
    if (checkCohesionTimeout(state.startTime)) return verifyCohesion(state);
    ...
    if (state.phases.phase3.status === 'failed') return verifyCohesion(state);
...
export type { SequentialHandshakeResult, VisualLivenessData, TactileIdentityData, VitalPulseData };
